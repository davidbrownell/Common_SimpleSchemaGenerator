...
class Plugin(PluginBase):
    ...
    # ----------------------------------------------------------------------
    @classmethod
    def _GeneratePolymorphicChildMap(cls, element):
        """
        Returns a map where the keys are unique attribute names and the value is the element
        for all elements derived from the provided element.
        """

        assert element.polymorphic
        assert not element.Base
        assert not element.IsDefinitionOnly

        # Get all the derived elements
        derived_elements = []
        ...
        while search:
            ...
            if elem.DerivedElements:
                search += elem.DerivedElements
                continue

            if elem not in derived_elements:
                derived_elements.append(elem)

        # Order the elements from most- to least-specific
        derived_elements.reverse()

        # Create the result
        result = OrderedDict()

        for derived_element in derived_elements:
            key = tuple(
                [
                    child.Name
                    for child in cls._EnumerateChildren(derived_element)
                    if not child.IsDefinitionOnly and not getattr(child, "IsAttribute", False) and child.TypeInfo.Arity.Min != 0
                ]
            )

            if key in map:
                raise Exception(
                    "The polymorphic derived element '{name} <{source} [{line}]>' is identical to '{prev_name} <{prev_source} [{prev_line}]>'".format(
                        name=derived_element.Name,
                        source=derived_element.Source,
                        line=derived_element.Line,
                        prev_name=result[key].Name,
                        prev_source=result[key].Source,
                        prev_line=result[key].Line,
                    )
                )

            result[key] = derived_element
